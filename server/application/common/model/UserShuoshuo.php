...
class UserShuoshuo extends Base
{
    ...
    {
        return db('user_shuoshuo');
    }

    //未删除
    const USER_SHUOSHUO_UNDELETE = 0;
    ...
    {
        $where['delete_time'] = self::USER_SHUOSHUO_UNDELETE;
    ...
    {
        $res = model('UserShuoshuoImg')->getAll(['user_shuoshuo_id' => $data['id']]);
        if (!$res) {
            return array();
        }
