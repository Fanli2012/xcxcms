...
class ShopLogic extends BaseLogic
{
    ...
    public function getOne($where = array(), $field = '*')
    {
        ...
        if (!empty($res['head_img'])) {
            $res['head_img'] = http_host() . $res['head_img'];
        }/* else{$res['head_img'] = http_host().'/images/avatar-loading.png';} */
        if (!empty($res['cover_img'])) {
            $res['cover_img'] = http_host() . $res['cover_img'];
        } else {
            $res['cover_img'] = http_host() . '/images/xcx-banner.jpg';
        }
        if (!empty($res['business_license_img'])) {
            $res['business_license_img'] = http_host() . $res['business_license_img'];
        }
        $res['wxacode'] = ''; //小程序码图片
    ...
    public function changePassword($data, $where = array())
    {
        ...
        if ($data['re_password'] != $data['password']) {
            return ReturnData::create(ReturnData::FAIL, null, '确认密码错误');
        }
        if (md5($data['password']) == $record['password']) {
            return ReturnData::create(ReturnData::FAIL, null, '新旧密码不能一致');
        }
        if (md5($data['old_password']) != $record['password']) {
            return ReturnData::create(ReturnData::FAIL, null, '旧密码错误');
        }
