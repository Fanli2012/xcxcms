...
class Server extends Controller
{
    ...
    public function listarc()
	{
        ...
        $list = db("article")->where($where)->field('body',true)->order("pubdate desc")->limit($limit)->select();
    ...
    public function detail()
	{
		...
		if(cache("detailid$id")){$post=cache("detailid$id");}else{$post = db('article')->where("id=$id")->find();cache("detailid$id",$post,2592000);}
		if($post)
        {
			$cat=$post['typeid'];
