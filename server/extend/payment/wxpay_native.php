...
class wxpay_native{
     ...
     * MCHID：商户号（必须配置，开户邮件中可查看或https://pay.weixin.qq.com 商户信息里查看）
    ...
    public function __construct($_config=array()){
        ...
        $paymentItem=$paymentModel->getPayment(['payment_code'=>'wxpay_native','payment_state'=>1]);
        if(!$paymentItem){
    ...
    public function unifiedOrder($Param)
    {
        ...
        //trade_type为NATIVE时，product_id为必填参数
        $data['product_id'] = $Param['pay_out_trade_no'];
